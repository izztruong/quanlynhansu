...
export const trainingLogsController = {
  async list(req: Request, res: Response) {
    ...
    const logs = await trainingLogsService.list(employeeId);
    res.json({ data: logs });
  },

  async getById(req: Request, res: Response) {
    const log = await trainingLogsService.getById(req.params.id);
    res.json({ data: log });
  },
  ...
  async update(req: Request, res: Response) {
    ...
    const log = await trainingLogsService.update(req.params.id, input);
    res.json({ data: log });
  },

  async remove(req: Request, res: Response) {
    await trainingLogsService.remove(req.params.id);
    res.status(204).send();
  },
